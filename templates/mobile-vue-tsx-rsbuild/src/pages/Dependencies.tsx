...
export default defineComponent(() => {
  ...
  const dependencySection = (title: string, dependencies: [string, string][]) => (
    <section class="dependency-section" aria-label={title}>
      <div class="dependency-heading">
        <TypographyTitle level="h2" content={title} />
        <span class="dependency-count">
          <TypographyText theme="secondary" content={`${dependencies.length} packages`} />
        </span>
      </div>
      <TCellGroup theme="card">
        {dependencies.map(([name, version]) => (
          <TCell
            ...
            key={name}
            title={() => <TypographyText content={name} strong />}
            note={() => (
              <span class="dependency-version">
                <TypographyText content={version} code copyable={{ text: version }} />
              </span>
            )}
          />
        ))}
    ...
    <main class="page-shell dependencies-page">
      ...
      <section class="dependencies-hero">
        <div class="dependencies-width">
          ...
          <TGrid class="dependencies-summary" column={3} border theme="card">
            <TGridItem
              image={() => <AppIcon size="24px" />}
              text={() => <TypographyTitle level="h3" content={`${runtimeDependencies.length}`} />}
              description={() => <TypographyText theme="secondary" content="Runtime" />}
            />
            <TGridItem
              image={() => <CodeIcon size="24px" />}
              text={() => (
                <TypographyTitle level="h3" content={`${developmentDependencies.length}`} />
              )}
              description={() => <TypographyText theme="secondary" content="Development" />}
            />
            <TGridItem
              image={() => <ComponentGridIcon size="24px" />}
              text={() => <TypographyTitle level="h3" content={`${totalDependencies}`} />}
              description={() => <TypographyText theme="secondary" content="Total" />}
            />
          </TGrid>
        </div>
      </section>
      <div class="dependencies-width dependencies-content">
        ...
        <TButton class="dependencies-home" variant="outline" size="large" block onClick={goHome}>
          Back to home
        </TButton>
      </div>
    </main>
