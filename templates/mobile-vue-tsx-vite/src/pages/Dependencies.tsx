...
export default defineComponent(() => {
  ...
  const dependencySection = (title: string, dependencies: [string, string][]) => (
    <section class="dependency-section" aria-label={title}>
      <div class="dependency-heading"><TypographyTitle level="h2" content={title} /><span class="dependency-count"><TypographyText theme="secondary" content={`${dependencies.length} packages`} /></span></div>
      <TCellGroup theme="card">{dependencies.map(([name, version]) => <TCell class="dependency-cell" key={name} title={() => <TypographyText content={name} strong />} note={() => <span class="dependency-version"><TypographyText content={version} code copyable={{ text: version }} /></span>} />)}</TCellGroup>
    </section>
  )
  ...
  return () => (
    <main class="page-shell dependencies-page">
      ...
      <div class="dependencies-width dependencies-content">
        ...
        <TButton class="dependencies-home" variant="outline" size="large" block onClick={goHome}>Back to home</TButton>
      </div>
    </main>
  )
})
