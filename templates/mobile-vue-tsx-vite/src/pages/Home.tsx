  ...
  return () => (
    <main class="page-shell">
      <TNavbar
        class="brand-navbar"
        fixed={false}
        left={() => <div class="brand-lockup"><img class="brand-logo" src={isDark.value ? logoDark : logoLight} alt="TDesign" /><TTag class="template-tag" theme="primary" variant="light">__TEMPLATENAME__</TTag></div>}
        right={() => <button class="theme-button" type="button" aria-label={themeLabel.value} title={themeLabel.value} onClick={(event) => { event.stopPropagation(); toggleTheme() }}>{isDark.value ? <SunnyIcon size="20px" /> : <MoonIcon size="20px" />}</button>}
      />
      ...
      <div class="content-width workspace">
        ...
        <section class="workspace-section" aria-labelledby="resources-title"><div class="section-title-row"><div><p class="section-kicker">Resources</p><h2 id="resources-title">Build from here</h2></div></div><TCellGroup theme="card">{resources.map((item) => <TCell key={item.url} title={item.title} description={item.description} arrow hover onClick={() => openLink(item.url)} />)}</TCellGroup></section>
